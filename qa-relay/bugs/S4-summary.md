...
### S4-009 (P1) — Sign Up 탭 클릭 시 제조사 가입 폼으로 전환
- buyer-app 내 .auth-tab "Sign Up" 클릭 → whistle-app.htm 제조사 가입 폼 로드
- 바이어 신규 가입 100% 차단
- S4-008과 동일 근본 원인: buyer-app 내부 이벤트가 cross-app navigation 트리거

### 재검증 결과
| 수정 | 상태 | 검증 내용 |
|------|------|----------|
| S6-fix-004 (제조사 앱 언어 전환기) | ✅ 확인 | 13개 언어 버튼 제조사 로그인에서 표시됨 |
| S6-fix-007 (buyer-app navigator.language 감지) | ✅ 확인 | 깨끗한 세션에서 영어 정상 표시 |
| S6-fix-008 (QA buyer role 교정) | ✅ 확인 | test-buyer-us 로그인 성공, DB role=buyer |
| TASK-005 (카테고리 확장) | ✅ 확인 | 12개 카테고리 드롭다운 표시 |

### 시각적 UX 최종 평가 (TASK-003)
| 페이지 | 데스크톱 | 모바일 | 평가 |
|--------|---------|--------|------|
| 바이어 랜딩 (/buyer) | ✅ 양호 | ✅ 양호 | 히어로 강력, 카테고리 명확, 인터랙티브 데모 인상적 |
| 바이어 로그인 | ✅ 양호 | ✅ 양호 | 깔끔한 폼, BUYER PORTAL 명확 |
| 바이어 대시보드 | ✅ 양호 | ⚠️ 쿠키배너 겹침 | 통계/파이프라인/활동 잘 구성 |
| 제품 검색 | ✅ 양호 | 미확인 | 카드 레이아웃 깔끔, 41개 결과 |
| 제품 상세 | ✅ 양호 | 미확인 | RFQ/샘플/저장 버튼 명확 |
...
| S4-009 | P1 | Sign Up 탭 클릭 → 제조사 가입 폼으로 전환 (바이어 가입 100% 차단) | OPEN |
| **S4-010** | **P0** | **buyer-app 내부 네비게이션 전면 깨짐 — 모든 서브페이지가 제조사 앱으로 전환** | **OPEN** |
| S4-visual-001 | P3 | 바이어 랜딩 "See It In Action" 로딩 스피너 빈 공간 | OPEN |
| S4-visual-002 | P2 | 바이어 대시보드 모바일 쿠키 배너 콘텐츠 가림 | OPEN |
