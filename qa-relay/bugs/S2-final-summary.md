...
1. **S2-bug-020 (P0)**: loadUser() 언어 매핑 — DB language 필드 또는 URL ?lang= 우선시
2. **S2-bug-014 (P0)**: T() fallback — 키 대신 영어 반환, _ML ja 번역 대규모 추가
3. **S2-bug-017 (P0)**: 메인 URL(/) → 랜딩 표시 (바이어 포털 리다이렉트 금지)
4. **S6-fix-005 배포 확인**: 쿠키 배너 + Contact Name 수정이 라이브에 미반영
5. **S2-bug-019 (P1)**: /ko_made 서비스 프리뷰 복구
6. **S2-bug-021 (P1)**: JPY 환율 계산 오류 ($18→¥18.18)
7. **S2-bug-023 (P1)**: 제품 등록 "Product Name (KR)" — JP 계정에 KR 표시
8. **S2-bug-024 (P1)**: "클릭" 한국어 하드코딩 — 비한국어 모드에서 노출
